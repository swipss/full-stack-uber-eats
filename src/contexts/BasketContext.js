...
const BasketContextProvider = ({ children }) => {
    ...
    useEffect(() => {
        if (basket) {
            DataStore.query(BasketDish, (basketDish) => basketDish.basketID("eq", basket.id)).then(
                setBasketDishes
            )
        }
    }, [basket])
    ...
    return (
        <BasketContext.Provider value={{ addDishToBasket, setRestaurant, basket, basketDishes, totalPrice }}>
            {children}
